...
This causes some problems regarding movement time of the motors of the device, which was covered in \autoref{sec:rts-intro}, in addition to issues regarding the intersection of target and projectile.
...
Based on the visual feed, as describes in \autoref{sec:obj_tracking}, it should be possible to calculate the movement of the target, however, this leaves the problem of finding the distance between target and device.

This problem produced several different solutions.
One solution, would be using a 3D camera, while another would be to use a \textit{ultrasonic sensor}, as presented in \autoref{sec:sensors}, and point it at the target.
