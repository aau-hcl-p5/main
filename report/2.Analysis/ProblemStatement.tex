...
All distances between the turret and the target are only based on the horizontal plane, not the actual distance including both the horizontal distance and the vertical distance.
...
This test case is similar to the \textit{dropping a ballon} test case, with the additional introduction of a projectile being shot from the \texttt{F.L.A.T}.
