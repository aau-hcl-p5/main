...
This means it can answer whether the image is of a car.
...
An alternative is called \textit{fast RCNN}, which is leaning toward being feasible for real time object detection.
% lacks text as well - hvad fuck er fast RCNN ? det skal da vist også stå her bro.
...
The framework is developed for Python, but has recently implemented Javascript support.
It is developed by the Google Brain team, and owned by Google Inc.\cite{tensorflow-attribution}.
It has a high-level API, called Keras, and a low-level API, which makes it possible to use it both as at an entry-level as well as making larger systems.
...
Tensors can also be tuples of different shapes, and when applying levels of abstraction, which is what Keras is doing, it is possible to use it for image recognition and other more complex problems.


\subsection{OpenCV}
OpenCV(Open Source Computer Vision Library), is a free-to-use library for C++, Python, and Java, which can be used for object tracking out of the box\cite{opencv}.

OpenCV is a rather old library, but it is still useable, and interfaces with other frameworks.
