...
\begin{lstlisting}[language=Python,label=lis:opencvExample,caption=OpenCV program to detect faces in an image.]
	import numpy as np
	import cv2
...
Below is an example of ML implemented using OpenCV:

\begin{lstlisting}[language=Python,label=lis:opencvMLExample,caption=OpenCV program recognize English letters\cite{opencvMLexamples}.]
	import cv2
	import numpy as np
...
These two examples are just a small sample of the capabilities of OpenCV.
