...
All connections in a neural network has a weight associated to it, which acts as a simple multiplier for the output from the previous neuron. Apart from the weights each neuron is assigned a bias, which is added to the sum of all inputs to the neuron. The bias is sometimes thought of as a sort of threshold indicating whether a neuron should fire.
