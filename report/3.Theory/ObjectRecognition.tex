...
Neural networks are a powerful, but complex, method of doing machine learning and simpler solutions should also be considered when exploring which method is best suited for solving the task.
...
All connections in a neural network has a weight associated with it, which acts as a simple multiplier for the output from the previous neuron. 
...
The bias is sometimes thought of as a threshold, that indicates if the neuron should fire.

A simple example of a neural network can be seen in Figure~\ref{fig:nnimag}.
...
This will be further discussed in Subsection~\ref{training-a-neural-network}, which discusses the training of a neural network.


The following paragraphs explains a few common activation functions.
\paragraph{Identity}
The simplest activation function, is an activation function that does not alter the input. 
This is also known as an identity function. 
The identity function can be used when learning simple models, but fails to learn complex non-linear models.
...
\paragraph{RELU}
Rectified linear unit is a modern yet simple activation function, that has proven to be very powerful in deep learning networks. 
...
To determine how much each weight and bias should be changed, the influence of each weight and bias is calculated. 
