...
As seen on Figure~\ref{fig:cyclicExecutive}, the tasks have a defined period and computation time, tasks are now placed into minor and major cycles in a way that they can all be run before their deadlines are met. 
...
In addition to the simple cyclic executive scheduling model, it is possible put parts of the program into tasks, and use a scheduler to determine which task should be run.
The most popular task-based scheduler is the fixed-priority scheduling method, which is based on each task having a fixed priority.
When using this scheduling method, each task is assigned a priority before run-time which cannot be changed.
The priority of a task is not an indication of the importance of the task it is a simply a priority to make the system schedulable\cite{RealTimeEmbeddedSystems}. 
...
Unlike fixed-priority scheduling, the absolute deadlines are computed at run-time, making it a dynamic scheduling model.
