...
The most commonly used task-based scheduler is the fixed-priority scheduling method, which is based on each task having a fixed priority, where the priority is assigned before run-time and cannot be changed at run-time\cite{RealTimeEmbeddedSystems}.
