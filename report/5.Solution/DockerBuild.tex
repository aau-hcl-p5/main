...
A test introduced, to reduce merge conflicts, as well as increasing readability.
...
One of the advantages of using a Continous Integration tool, is that the merging is self-testing.
This means, that when we maintain smaller code changes, that get merged into a mainline branch often, instead of monolithic merges, that have a higher likelyhood of conflicting code, and long running forks.

Overall the continous integration has helped us ensure, that unit tests always were passing, and that our mainline branch is working.
This is done by running through the flowchart in Figure~\ref{fig:flowchart-build}, every time some code needs to be build.
This includes building the NxtOSEK-program, ensuring no compiletime errors, and the \LaTeX{}-code as well.
