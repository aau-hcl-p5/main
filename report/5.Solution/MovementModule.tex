...
The move method makes use of the helper method \textt{get\_speed\_by\_distance}, as seen on Snippet~\ref{lst:getspeed}, which handles how fast the the motor should move.
...
In the first if sentence, it is checked whether the distance to the target is lower than a constant \textt{MOTOR_DEADZONE}, so that if the distance is smaller than the lowest amount of movement possible, it will return 0 and terminate.
